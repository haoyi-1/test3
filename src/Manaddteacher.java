...
public class Manaddteacher extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					Manaddteacher frame = new Manaddteacher();
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					int l = Integer.parseInt(s); // 工号
					String l1 = textField_1.getText();// 姓名
					String l2 = comboBox_1.getSelectedItem().toString();// 职称
					String l3 = textField_3.getText();// 院系
					String l5 = comboBox.getSelectedItem().toString();// 性别
					o1.init();
					if (!(textField.getText().matches("\\d{6}"))) {
						Component pan = null;
						JOptionPane.showMessageDialog(pan, "工号必须为6位数字！");
					} else {
						o1.insert("insert into teacher values('" + l + "','" + l1 + "','" + l5 + "','" + l2 + "','" + l3
								+ "','" + l + "')");
					}
					o1.close();
					 setVisible(false);
